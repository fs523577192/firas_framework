...
internal open class MutableBigInteger private constructor(
     ...
     * a one word capacity.
     */
    constructor(): this(IntArray(1), 0)

    /**
     * Construct a new MutableBigInteger with a magnitude specified by
     * the int val.
     */
    constructor(v: Int): this(IntArray(1), 1) {
        value[0] = v
    }
     ...
     */
    constructor(v: IntArray): this(v, v.size)

    /**
     * Construct a new MutableBigInteger with a magnitude equal to the
     * specified BigInteger.
     */
    constructor(b: BigInteger): this(b.mag, b.mag.size)

    /**
     * Construct a new MutableBigInteger with a magnitude equal to the
     * specified MutableBigInteger.
     */
    constructor(v: MutableBigInteger): this(v.value.copyOfRange(v.offset, v.offset + v.intLen), v.intLen)
    ...
    private val magnitudeArray: IntArray
        get() {
            if (offset > 0 || value.size != intLen)
                return value.copyOfRange(offset, offset + intLen)
            return value
        }
    ...
    private fun toLong(): Long {
        if (intLen > 2) throw ArithmeticException("this MutableBigInteger exceeds the range of long")
        if (intLen == 0)
            return 0
        val d = Integer.unsignedToLong(value[offset])
        return if (intLen == 2) d shl 32 or Integer.unsignedToLong(value[offset + 1]) else d
    }
    ...
    fun toBigInteger(sign: Int): BigInteger {
        ...
        return BigInteger(sign, magnitudeArray)
    }
    ...
    fun toBigDecimal(sign: Int, scale: Int): BigDecimal {
        ...
        if (len > 2 || d < 0 && len == 2)
            return BigDecimal(BigInteger(sign, mag), BigDecimal.INFLATED, scale, 0)
        val v = if (len == 2)
                Integer.unsignedToLong(mag[1]).or(Integer.unsignedToLong(d) shl 32)
            else
                Integer.unsignedToLong(d)
     ...
     * returns BigDecimal.INFLATED if value is not fit into long
     */
    ...
    fun compareHalf(b: MutableBigInteger): Int {
        ...
        while (i < len + offset) {
            val bv = bval[j++]
            val hb = Integer.unsignedToLong(bv.ushr(1) + carry)
            val v = Integer.unsignedToLong(`val`[i++])
            if (v != hb)
    ...
    fun leftShift(n: Int) {
        ...
        if (value.size < newLen) {
            ...
            // Use space on right
            for (i in 0..newLen - intLen - 1)
                value[offset + intLen + i] = 0
        } else {
            // Must use space on left
            for (i in 0..intLen - 1)
                value[i] = value[offset + i]
            for (i in intLen..newLen - 1)
                value[i] = 0
            offset = 0
        }
        intLen = newLen
    ...
    private fun divadd(a: IntArray, result: IntArray, offset: Int): Int {
        ...
        for (j in a.indices.reversed()) {
            val sum = Integer.unsignedToLong(a[j]) +
                    Integer.unsignedToLong(result[j + offset]) + carry
    ...
    private fun mulsub(q: IntArray, a: IntArray, x: Int, len: Int, offset: Int): Int {
        var offset = offset
        val xLong = Integer.unsignedToLong(x)
        var carry: Long = 0
        offset += len

        for (j in len - 1 downTo 0) {
            val product = Integer.unsignedToLong(a[j]) * xLong + carry
            val difference = q[offset] - product
            q[offset--] = difference.toInt()
            carry = product.ushr(32) + if (difference and BigInteger.LONG_MASK > Integer.unsignedToLong(product.toInt().inv()))
                1
            else
                0
        }
    ...
    private fun getLower(n: Int): BigInteger {
        if (isZero) {
            ...
            return BigInteger(sign, Arrays.copyOfRange(value, offset + intLen - len, offset + intLen))
        }
    }
    ...
    fun add(addend: MutableBigInteger) {
        ...
        while (x > 0 && y > 0) {
            x--
            y--
            sum = Integer.unsignedToLong(value[x + offset]) +
                    Integer.unsignedToLong(addend.value[y + addend.offset]) + carry
        ...
        while (x > 0) {
            ...
            if (carry == 0L && result == value && rstart == x + offset)
                return
            sum = Integer.unsignedToLong(value[x + offset]) + carry
            result[rstart--] = sum.toInt()
            carry = sum.ushr(32)
        }
        while (y > 0) {
            y--
            sum = Integer.unsignedToLong(addend.value[y + addend.offset]) + carry
            result[rstart--] = sum.toInt()
            carry = sum.ushr(32)
        }
    ...
    fun addShifted(addend: MutableBigInteger, n: Int) {
        ...
        while (x > 0 && y > 0) {
            ...
            sum = Integer.unsignedToLong(value[x + offset]) +
                    Integer.unsignedToLong(bval) + carry
            result[rstart--] = sum.toInt()
            carry = sum.ushr(32)
        }
        ...
        while (y > 0) {
            y--
            val bval = if (y + addend.offset < addend.value.size) addend.value[y + addend.offset] else 0
            sum = Integer.unsignedToLong(bval) + carry
            result[rstart--] = sum.toInt()
            carry = sum.ushr(32)
        }
    ...
    fun addDisjoint(addend: MutableBigInteger, n: Int) {
        ...
        // zero the gap
        for (i in rstart + 1 - y + len..rstart + 1 - 1)
            result[i] = 0
    ...
    fun subtract(b: MutableBigInteger): Int {
        ...
        while (y > 0) {
            ...
            diff = Integer.unsignedToLong(a.value[x + a.offset]) -
                    Integer.unsignedToLong(b.value[y + b.offset]) - (-(diff shr 32)).toInt()
            result[rstart--] = diff.toInt()
        }
        // Subtract remainder of longer number
        while (x > 0) {
            x--
            diff = Integer.unsignedToLong(a.value[x + a.offset]) - (-(diff shr 32)).toInt()
            result[rstart--] = diff.toInt()
        }
    ...
    private fun difference(b: MutableBigInteger): Int {
        ...
        while (y > 0) {
            x--
            y--
            diff = Integer.unsignedToLong(a.value[a.offset + x]) -
                    Integer.unsignedToLong(b.value[b.offset + y]) - (-(diff shr 32)).toInt()
            a.value[a.offset + x] = diff.toInt()
        }
        // Subtract remainder of longer number
        while (x > 0) {
            x--
            diff = Integer.unsignedToLong(a.value[a.offset + x]) - (-(diff shr 32)).toInt()
            a.value[a.offset + x] = diff.toInt()
        }
    ...
    fun multiply(y: MutableBigInteger, z: MutableBigInteger) {
        ...
        run {
            ...
            var k = yLen + xLen - 1
            while (j >= 0) {
                val product = Integer.unsignedToLong(y.value[j + y.offset]) * Integer.unsignedToLong(value[xLen - 1 + offset]) + carry
        ...
        for (i in xLen - 2 downTo 0) {
            ...
            var k = yLen + i
            while (j >= 0) {
                val product = Integer.unsignedToLong(y.value[j + y.offset]) * Integer.unsignedToLong(value[i + offset]) +
                        Integer.unsignedToLong(z.value[k]) + carry
                z.value[k] = product.toInt()
                carry = product.ushr(32)
                j--
                k--
    ...
    fun mul(y: Int, z: MutableBigInteger) {
        ...
        // Perform the multiplication word by word
        val ylong = Integer.unsignedToLong(y)
        ...
        for (i in intLen - 1 downTo 0) {
            val product = ylong * Integer.unsignedToLong(value[i + offset]) + carry
    ...
    fun divideOneWord(divisor: Int, quotient: MutableBigInteger): Int {
        val divisorLong = Integer.unsignedToLong(divisor)

        // Special case of one word dividend
        if (intLen == 1) {
            val dividendValue = Integer.unsignedToLong(value[offset])
        ...
        var remLong = Integer.unsignedToLong(rem)
        if (remLong < divisorLong) {
            quotient.value[0] = 0
        } else {
            quotient.value[0] = (remLong / divisorLong).toInt()
            rem = (remLong - quotient.value[0] * divisorLong).toInt()
            remLong = Integer.unsignedToLong(rem)
        }
        var xlen = intLen
        while (--xlen > 0) {
            val dividendEstimate = remLong shl 32 or Integer.unsignedToLong(value[offset + intLen - xlen])
            val q: Int
     ...
     */
    fun divideKnuth(b: MutableBigInteger, quotient: MutableBigInteger, needRemainder: Boolean = true): MutableBigInteger? {
        var b = b
        if (b.intLen == 0)
    ...
    fun divide(v: Long, quotient: MutableBigInteger): Long {
        ...
        if (d == 0)
            return Integer.unsignedToLong(divideOneWord(v.toInt(), quotient))
        else {
    ...
    private fun divideMagnitude(div: MutableBigInteger,
        ...
        val dh = divisor[0]
        val dhLong = Integer.unsignedToLong(dh)
        val dl = divisor[1]

        // D2 Initialize j
        for (j in 0..limit - 1 - 1) {
            // D3 Calculate qhat
            // estimate qhat
            var qhat = 0
            var qrem = 0
            ...
            if (nh == dh) {
                qhat = 0.inv()
                qrem = nh + nm
                skipCorrection = qrem + 0x80000000.toInt() < nh2
            } else {
                val nChunk = nh.toLong() shl 32 or Integer.unsignedToLong(nm)
                if (nChunk >= 0) {
            ...
            if (!skipCorrection) { // Correct qhat
                val nl = Integer.unsignedToLong(rem.value[j + 2 + rem.offset])
                var rs = Integer.unsignedToLong(qrem).shl(32).or(nl)
                var estProduct = Integer.unsignedToLong(dl) * Integer.unsignedToLong(qhat)

                if (unsignedLongCompare(estProduct, rs)) {
                    qhat--
                    qrem = Integer.unsignedToLong(qrem).plus(dhLong).toInt()
                    if (Integer.unsignedToLong(qrem) >= dhLong) {
                        estProduct -= Integer.unsignedToLong(dl)
                        rs = Integer.unsignedToLong(qrem).shl(32) or nl
                        if (unsignedLongCompare(estProduct, rs))
                            qhat--
                    }
                }
            ...
            // D5 Test remainder
            if (borrow + 0x80000000.toInt() > nh2) {
                // D6 Add back
        ...
        if (qhat != 0) {
            if (!skipCorrection) { // Correct qhat
                val nl = Integer.unsignedToLong(rem.value[limit + 1 + rem.offset])
                var rs = Integer.unsignedToLong(qrem).shl(32).or(nl)
                var estProduct = Integer.unsignedToLong(dl) * Integer.unsignedToLong(qhat)
    ...
    private fun divideLongMagnitude(ldivisor: Long, quotient: MutableBigInteger): MutableBigInteger {
        ...
        // D1 normalize the divisor
        val shift = Integer.numberOfLeadingZeros(ldivisor)
        if (shift > 0) {
    ...
    private fun divaddLong(dh: Int, dl: Int, result: IntArray, offset: Int): Int {
        ...
        var sum = Integer.unsignedToLong(dl) + Integer.unsignedToLong(result[1 + offset])
        result[1 + offset] = sum.toInt()

        sum = Integer.unsignedToLong(dh) + Integer.unsignedToLong(result[offset]) + carry
    ...
    private fun mulsubLong(q: IntArray, dh: Int, dl: Int, x: Int, offset: Int): Int {
        ...
        val xLong = Integer.unsignedToLong(x)
        offset += 2
        var product = Integer.unsignedToLong(dl) * xLong
        var difference = q[offset] - product
        q[offset--] = difference.toInt()
        var carry = product.ushr(32) +
                if (difference.and(BigInteger.LONG_MASK) > Integer.unsignedToLong(product.toInt().inv())) 1
                else 0
        product = Integer.unsignedToLong(dh) * xLong + carry
    ...
    fun modInverseMP2(k: Int): MutableBigInteger {
        ...
        var pLong = Integer.unsignedToLong(value[offset + intLen - 1])
        if (intLen > 1)
            pLong = pLong or (value[offset + intLen - 2].toLong() shl 32)
        var tLong = Integer.unsignedToLong(t)
    ...
    companion object {
        ...
        fun divWord(n: Long, d: Int): Long {
            val dLong = Integer.unsignedToLong(d)
            var r: Long
            var q: Long
            if (dLong == 1L) {
                q = n.toInt().toLong()
        ...
        fun binaryGcd(a: Int, b: Int): Int {
            ...
            while (a != b) {
                if (a + 0x80000000.toInt() > b + 0x80000000.toInt()) {  // a > b as unsigned
                    a -= b
